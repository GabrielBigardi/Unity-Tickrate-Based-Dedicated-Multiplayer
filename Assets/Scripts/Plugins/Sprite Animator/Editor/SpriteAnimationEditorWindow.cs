    ...
    public class SpriteAnimationEditorWindow : ExtendedEditorWindow
    {
        ...
        private void DrawAnimationPreview()
        {
            ...
            if (framesProperty != null && framesProperty.arraySize - 1 >= _currentFrame)
            {
                ...
                if (sprite != null)
                {
                    GUIDrawSpriteFixed(textureRect, sprite);
                }
            }
            EditorGUILayout.EndVertical();
        }
        ...
        public void GUIDrawSpriteFixed(Rect rect, Sprite sprite)
        {
            float aspectRatio = sprite.rect.width / sprite.rect.height;
            float textureRectWidth = rect.width;
            float textureRectHeight = rect.height;
            if (aspectRatio > 1)
            {
                textureRectHeight /= aspectRatio;
            }
            else
            {
                textureRectWidth *= aspectRatio;
            }
            float textureRectX = rect.x + (rect.width - textureRectWidth) / 2f;
            float textureRectY = rect.y + (rect.height - textureRectHeight) / 2f;
            Rect textureRect = new(textureRectX, textureRectY, textureRectWidth, textureRectHeight);
            Rect spriteRect = sprite.rect;
            Texture2D tex = sprite.texture;
            GUI.DrawTextureWithTexCoords(textureRect, tex, new Rect(spriteRect.x / tex.width, spriteRect.y / tex.height, spriteRect.width / tex.width, spriteRect.height / tex.height));
        }
